  ...
  var angleInc = 5.0;
  const procImageWidth = 64;
  const procImageHeight = 64;

  function doLoad(theTexture, theSource) {
      gl.bindTexture(gl.TEXTURE_2D, theTexture);
      if (theSource instanceof HTMLImageElement) {
          gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, theSource);
      } else if (theSource instanceof Uint8Array) {
          const width = procImageWidth;
          const height = procImageHeight;
          gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, theSource);
      }
...
function setUpTextures(){
    ...
    const testFace = new Image();
    testFace.src = 'test_face.jpg';

    worldImage.onload = () => {
        doLoad (worldTexture, worldImage);
    };
    testFace.onload = () => {
      doLoad(faceTexture, testFace);
    }

    const randomTextureData = generateRandomTexture(procImageWidth, procImageHeight);

    doLoad(procTexture, randomTextureData);
}
...
function drawCurrentShape () {
    ...
    if (curTexture === "globe") {
        ...
        console.warn("Unknown texture type:", curTexture);
        gl.bindTexture (gl.TEXTURE_2D, worldTexture);
    }
...
function generateRandomTexture(width, height) {
  const data = new Uint8Array(width * height * 4); // RGBA for each pixel
  for (let i = 0; i < width * height * 4; i++) {
      data[i] = Math.floor(Math.random() * 256); // Random value between 0 and 255
  }
  return data;
}
